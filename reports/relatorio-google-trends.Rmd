...
p <- props_nome_formal %>%  ggplot() + 
  ...
  xlab("Tempo") + 
  ylab("Pressão") +
  ggtitle("Evolução da pressão ao longo dos dias") +
  theme_minimal() +
  facet_grid(rows = vars(nome_formal)) 
...
p <- semana %>%  ggplot() + 
  ...
  xlab("Tempo") + 
  ylab("Pressão") +
  ggtitle("Evolução da pressão ao longo das semanas") +
  theme_minimal() +
  facet_grid(rows = vars(nome_formal)) 
...
p <- semana %>%  ggplot() + 
  ...
  xlab("Tempo") + 
  ylab("Pressão") +
  ggtitle("Evolução da pressão da PLS 45/2017") +
  theme_minimal()
...
```{r}
temas_semana <- temas_prop_nome_formal %>% 
  mutate(semana = epiweek(date)) %>% 
  group_by(tema, semana) %>% 
  summarise(pop_semana = max(pop))

p <- temas_semana %>%  ggplot() + 
  geom_area(aes(x=semana, y = pop_semana, fill = tema)) +
  xlab("Tempo") + 
  ylab("Pressão") +
  ggtitle("Evolução da pressão por tema ao longo das semanas") +
  theme_minimal() +
  facet_grid(rows = vars(tema)) 
...
p <- props_apelido %>%  ggplot() + 
  ...
  ggtitle("Evolução da pressão dos apelidos ao longo dos dias") +
...
p <- semana_apelido %>%  ggplot() + 
  ...
  ggplot2::xlab("Tempo") + 
  ggplot2::ylab("Pressão") +
  ggtitle("Evolução da pressão dos apelidos ao longo das semanas") +
  ggplot2::theme_minimal() +
  ggplot2::facet_grid(rows = vars(apelido)) 
...
A seguir, podemos visualizar a pressão atual e da semana anterior para as duas proposições:
```{r}
