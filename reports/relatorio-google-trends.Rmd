...
p <- hist_previdencia %>%  ggplot() + 
  geom_area(aes(x=as.Date(periodo), y = temperatura_recente), fill = "#dc6060") +
  xlab("Semana") + 
  ylab("Temperatura") +
  ggtitle("Evolução da Temperatura da PEC 06/2019 - Nova previdência") +
  scale_x_date(date_labels = "%d-%m-%Y") +
  theme_minimal()
ggplotly(p)
```
...
**Pico semana 06/10:** Houve algumas notícias sobre a previdência [OGlobo](https://oglobo.globo.com/economia/governo-dispensa-287-tipos-de-empresas-de-alvaras-de-funcionamento-23735455)

**Pico emana 08/07:** Aprovação na Comissão Mista

**Pico emana 05/08:** [Novo texto](https://exame.abril.com.br/economia/novo-texto-da-mp-da-liberdade-economica-inclui-trabalho-aos-domingos/)
...
p <- hist_lib_economica %>%  ggplot() + 
  geom_area(aes(x=as.Date(periodo), y = temperatura_recente), fill = "#dc6060") +
  xlab("Semana") + 
  ylab("Temperatura") +
  ggtitle("Evolução da Temperatura da MPV 881/2019 - Liberdade Econômica") +
  scale_x_date(date_labels = "%d-%m-%Y") +
  theme_minimal()
ggplotly(p)
```
...
p <- hist_mercado_agua %>%  ggplot() + 
  geom_area(aes(x=as.Date(periodo), y = temperatura_recente), fill = "#dc6060") +
  xlab("Semana") + 
  ylab("Temperatura") +
  ggtitle("Evolução da Temperatura da PLS 495/2017 - Mercado da Água") +
  scale_x_date(date_labels = "%d-%m-%Y") +
  theme_minimal()
ggplotly(p)
```
