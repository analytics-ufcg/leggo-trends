...
.get_parliamentarians_usernames <- function() {
  ...
  source(here("scripts/utils/constants.R"))
...
#' atividade no Twitter nos últimos 90 dias.
#' @return Dataframe com parlamentares ativos no Twitter nos últimos 90 dias.
.filter_active_users <- function(usernames_list) {
  ...
  active_users <- users_df %>%
    dplyr::filter(created_at >= last_90_days) %>%
    dplyr::select(user_id, username = screen_name)
  
  return(active_users)
}

#' @title Retorna os dados de entrada para a função do leggoTrends que retorna os tweets dos últimos 90 dias.
...
#' @description A partir de uma planilha do Drive, retorna os dados de usernames dos parlamentares no twitter.
#' @return Dataframe com usernames de parlamentares no twitter
search_last_30_days <- function(words_df, from_date, to_date) {
  library(tidyverse)
  ...
  tweets <- purrr::map_df(
    ...
    ~ leggoTrends::get_tweets_pls_last_90_days(authors_query = .x, 
